...
<?php
  ...
  $rent_categories_terms = get_the_terms( $post_id, 'rent_categories' );
  if ( ! $rent_categories_terms ) {
    $rent_categories_terms = array();
  }

  $cat = null;
  $subcat = null;
  foreach ( $rent_categories_terms as $term ) {
    if ( 0 == $term->parent ) {
      $cat = $term->term_id;
      break;
    }
...
<div class="edit-rent__as"<?php if ( 'rent' != $post_type ) echo ' style="display: none;"'; ?>>
  <form id="edit-rent__as" method="post" action="<?php echo admin_url( 'admin-post.php' ); ?>">
    ...
    <div class="item__as">
      <div class="content__as">
        ...
        <div class="btns__as tender-type__as">
          ...
          <button type="button" class="active__as" data-block=".edit-rent__as">Аренда</button>
        ...
        <select class="cat__as" name="as_cat" data-placeholder="Тип оборудования/техники" data-validation="required">
          ...
          <optgroup label="Популярные">
            <?php foreach ( $rent_categories_parents_popular as $rent_category ): ?>
              <option value="<?php echo $rent_category->term_id; ?>"<?php if ( $cat == $rent_category->term_id ) echo ' selected'; ?>><?php echo $rent_category->name; ?></option>
            <?php endforeach; ?>
          </optgroup>
          <optgroup label="Все">
    ...
    <div class="item__as item-2__as">
      <div class="content__as">
        ...
        <select class="subcat__as" name="as_subcat" data-placeholder="Категория"<?php if ( 'rent' != $post_type ) echo ' disabled'; ?>>
          ...
          <optgroup label="Популярные" class="popular__as">
            <?php foreach ( $rent_categories_childs_popular as $rent_category ): ?>
              <option value="<?php echo $rent_category->term_id; ?>" data-parent="<?php echo $rent_category->parent; ?>"<?php if ( $subcat == $rent_category->term_id ) echo ' selected'; ?><?php if ( ( ! $subcat_parent && $cat != $rent_category->parent ) || ( $subcat_parent && $subcat_parent != $rent_category->parent ) ) echo ' disabled'; ?>><?php echo $rent_category->name; ?></option>
            <?php endforeach; ?>
          </optgroup>
