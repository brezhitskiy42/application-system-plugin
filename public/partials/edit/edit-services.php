...
<?php
  ...
  $services_categories_terms = get_the_terms( $post_id, 'services_categories' );
  if ( ! $services_categories_terms ) {
    $services_categories_terms = array();
  }

  $cat = null;
  foreach ( $services_categories_terms as $term ) {
    if ( 0 == $term->parent ) {
      $cat = $term->term_id;
      break;
    }
...
<div class="edit-services__as"<?php if ( 'services' != $post_type ) echo ' style="display: none;"'; ?>>
  <form id="edit-services__as" method="post" action="<?php echo admin_url( 'admin-post.php' ); ?>">
    ...
    <div class="item__as">
      <div class="content__as">
        ...
        <div class="btns__as tender-type__as">
          ...
          <button type="button" class="active__as" data-block=".edit-services__as">Услуги</button>
        </div>
      </div>
      ...
      <div class="content__as">
        ...
        <select class="main-cat__as" name="as_cat" data-placeholder="Тип услуг" data-validation="required">
          ...
          <optgroup label="Популярные">
            <?php foreach ( $services_categories_parents_popular as $services_category ): ?>
              <option value="<?php echo $services_category->term_id; ?>"<?php if ( $cat == $services_category->term_id ) echo ' selected'; ?>><?php echo $services_category->name; ?></option>
            <?php endforeach; ?>
          </optgroup>
          <optgroup label="Все">
        ...
        <div class="preset-links__as">
          <?php foreach ( $services_categories_parents_presets as $services_category ): ?>
            <span class="link__as" data-id="<?php echo $services_category->term_id; ?>" data-select=".cat__as"><?php echo $services_category->name; ?></span>
          <?php endforeach; ?>
        </div>
      </div>
      <div class="sidebar__as">
